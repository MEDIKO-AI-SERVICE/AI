...
class PharmacyRecommender:
    ...
    def recommend_pharmacies(self, pharmacies_df, member_id=None):
        ...
        #3. 로그가 없는 경우: 이동시간+운영시간을 반영한 점수로 추천
        if not has_user_logs:
            print(f"No user logs found for member_id: {member_id}, using distance+operating_hours ranking")
            pharmacies_df['similarity'] = pharmacies_df.apply(lambda row: self.calculate_reward(row, None), axis=1)
            recommended = pharmacies_df.nlargest(self.K, 'similarity')
            recommended = recommended.drop(columns=['base_score'])
            return recommended
        
        #4. 로그가 있는 경우: 기존 강화학습 로직 사용
        ...
        #5. Top-K Off-Policy Correction 적용
        ...
        #8. 최종 점수 계산 (보상 * 중요도 가중치) - similarity 컬럼에 저장
